...
char PcdRequest(u8 req_code,u8 *pTagType)
{
	...
	ClearBitMask(Status2Reg,0x08);//清所有中断位
	WriteRawRC(BitFramingReg,0x07);
	SetBitMask(TxControlReg,0x03);//清FIFO缓存
    	 ...
    	 for (i=0; i<4; i++)
			 {   
					 *(pSnr+i)  = ucComMF522Buf[i];
					 snr_check ^= ucComMF522Buf[i];//ucComMF522Buf[i] 111 0101  ^ 异或  不同为1，相同为0
			 }
...
char PcdSelect(u8 *pSnr)
{
    ...
    for (i=0; i<4; i++)
    {
    	...
    	ucComMF522Buf[6]  ^= *(pSnr+i);//ucComMF522Buf[6]为pSnr[3]的值
    }
		//用MF522计算CRC16函数  void CalulateCRC(u8 *pIn ,u8   len,u8 *pOut )
...
//		}
    ...
    if ((status == MI_OK) && (unLen == 0x18))// 0x18  24  3个字节
    {   status = MI_OK;  }
    else
    {   status = MI_ERR;    }
...
//    {    ucComMF522Buf[i+8] = *(pSnr+i);   }
	...
	//将源存储块（src）的前n个字节复制到目的存储块（dest）中。
...
u8 AddAccessM1(u8 mode)
{
	...
	{
		printf("寻卡成功\r\n");
		if (PcdAnticoll(SN) != MI_OK)    return 1;//防衝撞失败
		printf("防冲撞成功\r\n");
		if (PcdSelect(SN) != MI_OK)    return 1;//选卡失败	
		printf("选卡成功\r\n");
		if (PcdAuthState(0x60,7,KEY,SN) != MI_OK)    return 1;//驗證失败
		printf("验证成功\r\n");		
		
		if(mode == 0)
		{
			PassWord = SN[0]<<9 | SN[1]<<10 | SN[2]<<11 | SN[3]<<12;//简单算法加密		
...
AA:
		...
		printf("CT[1]=%d\r\n",CT[1]);
		printf("status=%d\r\n",status);
		while( status!=MI_OK)//等待寻卡
		{
			goto AA;
		}
		printf("status=%d\r\n",status);
	while(status==MI_OK)//尋卡成功
	{
		...
		{		
			...
			{	
				printf("选卡\r\n");	
				print_info(SN,4);
				if (PcdAuthState(0x60,1,KEY,SN)== MI_OK)//驗證成功
					{	
						printf("验证成功\r\n");	
						printf("KEY\r\n");	
						print_info(KEY,6);
						...
						printf("SN2\r\n");
						print_info(SNT,4);
						if(PcdRead(1,SNT)==MI_OK)//读卡
							{		
								printf("SN3\r\n");
								print_info(SN,4);
								printf("card_2\r\n");
								print_info((char *)card_2,4);
								if(strncmp((char *)SN,(char *)card_2,4)==0)//判断卡号是否匹配
								{
										RC522_Rtc_flag=0;
										rc522_error_i=0;
										BEEP=1;
